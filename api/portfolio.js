...
export default async function handler(req, res) {
    ...
    if (req.method === "GET") {
        try {
            ...
            const response = await fetch(
                `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/${SHEET_NAME}!A:H`,
            ...
            const portfolios = rows.slice(1).map((row, index) => ({
                ...
                kategori: row[5] || "",
                tanggal: row[6] || "",
                gambar_detail: row[7] || "", // pipe-separated URLs
            }));
    ...
    if (req.method === "POST") {
        const { judul, deskripsi, gambar, link, teknologi, kategori, gambar_detail } = req.body;

        if (!judul || !deskripsi || !gambar) {
            return res.status(400).json({ error: "Judul, deskripsi, dan thumbnail wajib diisi." });
        }
        ...
        // gambar_detail: array of URLs → join dengan pipe "|"
        const detailStr = Array.isArray(gambar_detail)
            ? gambar_detail.filter(Boolean).join("|")
            : (gambar_detail || "");
        ...
        try {
            ...
            const response = await fetch(
                `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/${SHEET_NAME}!A:H:append?valueInputOption=USER_ENTERED`,
                {
                    ...
                    body: JSON.stringify({
                        values: [[judul, deskripsi, gambar, link || "", teknologi || "", kategori || "", tanggal, detailStr]],
                    }),
                }
            );
...
// ── Helper: pastikan header row ada ──────────────────────────────────────────
async function ensurePortfolioHeader(token, spreadsheetId, sheetName) {
    const checkRes = await fetch(
        `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${sheetName}!A1:H1`,
        { headers: { Authorization: `Bearer ${token}` } }
    );
    ...
    if (!firstRow || firstRow[0] !== "Judul") {
        await fetch(
            `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${sheetName}!A1:H1?valueInputOption=USER_ENTERED`,
            {
                method: "PUT",
                headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
                body: JSON.stringify({
                    values: [["Judul", "Deskripsi", "Gambar Thumbnail", "Link Project", "Teknologi", "Kategori", "Tanggal", "Gambar Detail"]],
                }),
            }
        );
    }
